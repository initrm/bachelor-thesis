...
migliorando la complessità temporale dei precendenti.

Tutte le tecniche note condividono lo schema ad alto livello riportato nella Sezione
\ref{sec:hyperminimization-strategy} per poi specializzarsi, in
maniera differente sul come le singole operazioni vengono svolte.
...
In altri termini, trattando l'automa come un grafo, il problema può essere ricondotto, per ogni coppia di stati
$q_i, q_j \in Q$ alla ricerca di un cammino di lunghezza maggiore di 1 tra $q_i$ e $q_j$. Sono note diverse
tecniche per risolvere il problema della ricerca di cammini in un grafo \cite{DFI08}, un possibile 
approccio potrebbe essere quello di utilizzare una visita in ampiezza (Breadth-First Search).
...
Si procede con la determinazione delle classi di quasi-equivalenza, a questo scopo, viene creato un vettore
...
L'algoritmo proposto da Andrew Badr \cite{Badr} migliora la complessità algoritmica dell'algoritmo
di Badr, Geffert e Shipman cambiando quella che è la strategia utilizzata per identificare le classi di
quasi-equivalenza.

%La differenza sostanziale risiede nel modo in cui vengono identificate le classi di quasi-equivalenza.
...
\begin{figure}[hbt!]
  \centering
  \includegraphics[width=0.6\linewidth]{images/xcp_start.png}
