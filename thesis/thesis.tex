...
\begin{lemma}\label{lem:kernel-state}
  Uno stato $q_1 \in Q$ è in $K(A)$ se e solo se può essere raggiunto dallo stato iniziale $q_I$ attraverso
  un cammino passante per uno stato $q_2$ tale che esiste un ciclo in $A$ che inizia e termina in $q_2$.
\end{lemma}
...
\begin{enumerate}
  ...
  \item ottenere l'automa $A''$, $\forall Q_i \in \partitioned{Q'}{\sim}$ tale che $||Q_i|| > 1$ ed $\exists q_{i_1} \in Q_i$ tale che $q_{i_1} \in P(A')$:
  \begin{enumerate}
    \item se $\exists q_{i_2} \in Q_i$ tale che $q_{i_2} \in K(A')$, collassando tutti gli stati in $Q_i \cap P(A')$ in $q_{i_2}$
    \item altrimenti, collassando tutti gli stati in $Q_i \setminus \{q_{i_1}\}$ in $q_{i_1}$
  \end{enumerate}
\end{enumerate}
...
che l'automa minimo ottenuto $A'$, sul quale poi eseguire i successivi passi per arrivare all'automa iperminimo,
sia privo di stati irraggiungibili ($U(A') = \emptyset$) e che quindi l'insieme degli stati possa essere partizionato
...
Sono noti diversi algoritmi per lo svolgimento di tale compito \cite{DFI08}, un possibile approccio potrebbe
essere quello di utilizzare l'algoritmo di Floyd-Warshall, che richiede tempo $\bigo(n^3)$.
...
un vettore $V$ di dimensione $||Q'||$ tale che $v_i = 1$ se e solo se $q_i \in K(A')$, altrimenti $v_i = 0$.
Grazie al Lemma \ref{lem:kernel-state} la costruzione del vettore $V$ si riduce nel determinare se per ogni
stato $q_i \in Q'$, esista uno stato $q_j \in Q'$ tale che $e_{1,j} = e_{j,j} = e_{j,i} = 1$.

É banale dunque osservare come la costruzione del vettore $V$ sia eseguita in tempo $\bigo(n)$ essendo che 
...
mentre i vettori $V$ dei kernel e $R$ delle classi di quasi-equivalenza risulterebbero essere:

\begin{center}
  $V = \begin{bNiceMatrix}[last-row]
    0 & 0 & 0 & 0 & 1 & 1 & 1 \\
    \scriptstyle{A} & \scriptstyle{B} & \scriptstyle{C} & \scriptstyle{D} & \scriptstyle{E} & \scriptstyle{F} & \scriptstyle{G}
  \end{bNiceMatrix}$, 
  \
...
una nuova operazione tra automi a stati finiti deterministici.
...
Un esempio di XOR cross product è quello dell'automa in Figura \ref{fig:xcp_start} con se stesso, il cui 
risultato è riportato in figura \ref{fig:xcp_end}.

\begin{figure}
  \centering
  \includegraphics[width=0.5\linewidth]{images/xcp_start.png}
  \caption{\label{fig:xcp_start}Un ulteriore esempio automa a stati finiti deterministico.}
\end{figure}
\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{images/xcp_end.png}
  \caption{\label{fig:xcp_end}L'automa in Figura \ref{fig:xcp_start} in XOR cross product con se stesso.}
\end{figure}

\begin{theorem}\label{th:cross-prod-state-aeq}
  Siano $A = (Q, \Sigma, \delta, q_I, F)$ un automa a stati finiti deterministico e 
  $A^\otimes = (Q^\otimes, \Sigma, \delta^\otimes, q_{I}^\otimes, F^\otimes)$ l'automa risultante dallo 
  XOR cross product di $A$ con se stesso, allora: $\forall w \in \Sigma^*$ e $\forall (q, r) \in Q^\otimes$
  si ha $w \in L((q, r)) \Leftrightarrow \delta^\otimes((q, r), w) \in F^\otimes \Leftrightarrow 
  (\delta(q, w) \in F) \otimes (\delta(r, w) \in F)$, dove $L(q)$ è il linguaggio indotto dallo stato $q$.
  In altri termini, il linguaggio $L((q, r))$ di ogni stato nel contesto $A^\otimes$, è uguale al linguaggio
  $L(q) \Delta L(r)$ nel contesto $A$.
\end{theorem}
...
di $T^\rightarrow_q$ e $T^\leftarrow_q$ per tutti gli stati corrisponde complessivamente a $\bigo(n)$, inoltre,
l'aggiunta di ogni stato ad $R$ richiede un tempo costante $\bigo(1)$ più un tempo trascurabile per ciascuno
stato da cui si raggiunge quello corrente, pertanto la costruzione di $R$ richiede anch'essa un tempo lineare $\bigo(n)$.

La costruzione di $\partitioned{Q'}{\sim}$ avviene ora semplicemente, utilizzando una struttura dati QuickUnion $U$ con 
union-by-rank e compressione del cammino \cite{DFI08}. La struttura viene inizializzata per rappresentare la partizione
di $Q'$ composta esclusivamente da singoletti, quindi, per ogni stato $(q, r) \in R$, siano $Q_i$ e $Q_j$
rispettivamente i risultati delle operazioni di $find(q)$ e $find(r)$ in $U$, se $Q_i \neq Q_j$, 
allora si uniscono gli insiemi $Q_i$ e $Q_j$ tramite l'operazione $union(Q_i, Q_j)$ in $U$. 


Essendo per ogni (q, p) in R q quasi equivalente P grazie a corollario 2.2, al termine dell'operazione bla bla

In questo modo, 
seguendo quanto riportato nel Corollario \ref{cor:cross-prod-state-aeq-states}, al termine dell'operazione, $U$ rappresenterà
$\partitioned{Q'}{\sim}$.

% For every word w ∈ Σ∗ and state (q, r) ∈ Q⊗ we have w ∈ L((q, r)) ⇔ δ⊗((q, r), w) ∈ A⊗ ⇔ (δ(q, w) ∈ A)⊗(δ(r, w) ∈ A) by definition. In other words, the language L((q, r)) of every state in the D⊗ context equals the language L(q) △ L(r) in the D context. The first consequence of this is that S is exactly the set of states in D⊗ that induce the empty language, because the given DFA D is minimized (so no two distinct states induce the same language). This proves that the input to right finite states is correct, and the result F is as desired. Thus, as a second consequence of the above, we see that L((q, r)) is finite if and only if q ∼ r. There- fore, F is the f-equivalence relation on the states in D. Step 4 turns this relation, represented as a set of pairs, into a partition.

, si uniscono le classi $Q_i$ e $Q_j$ tramite $U.union(Q_i, Q_j)$.
% 
% For each (q,r) in F:
% i. Let Pq = P.find(q) ii. Let Pr = P.find(r)
% iii. If Pr ̸= Pq, then P.union(Pq,Pr)
% Use the state-pairs in F to construct a partition P of Q:
% (1) Let P be a new Union-Find data structure
% (2) Foreachstateq∈Q:makeanewset{q}inU (3) For each (q,r) in F:
% i. Let Pq = P.find(q) ii. Let Pr = P.find(r)
% iii. If Pr ̸= Pq, then P.union(Pq,Pr) Return P
% 
% è chiaro come questa sia una maniera alternativa di fare quanto fs algo uno con matrice e

% In altre parole, gli stati che inducono un linguaggio finito sono gli stati che possono essere raggiunti
% a partire dallo stato iniziale seguendo una sequenza di transizioni etichettate con stringhe finite.


% First, we prove that if a state is removed, then it induces a finite language. 
% Note that there are no transitions from S to a state in S′. 
% Otherwise, the state in S′ would also induce an empty language, so by assumption it would be in S. 
% Next, assign to each removed state q a distance d(q) from S, equal to the length of the longest path from q 
% to a state in S. We obtain the result by induction on d. If d(q) = 0, then q ∈ S and |L(q)| = 0 by definition. 
% The size of L(q) for any state q in a DFA is bounded above by 1 + Σq′∈Outgoingq L(q′). Therefore, if all 
% removed states with d <= n induce finite languages, it follows that all states with d = n + 1 also induce a
% finite language, completing this direction.
% Second, we prove that if a state q induces a finite language, it is removed by the algorithm. Again we use a simple inductive proof. Let l(q) be the length of some longest word w in L(q). If l(q) = 0 then q is in S, so it is removed by the algorithm. If l = n, then for every state q′ ∈ Outgoingq, l(q′) < l(q). Therefore, if every state with l <= n is removed by the algorithm, then every state with l = n + 1 is also removed, because all states it transitions to are removed.

\subsection{Algoritmo di Holzer e Maletti}

% \chapter{Algoritmi di iperminimizzazione}
