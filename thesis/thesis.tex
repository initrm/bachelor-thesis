...
\begin{algo}{Algoritmo per la costruzione dell'automa iperminimo.}
  ...
  \For{$i \gets 1$ \KwTo $||Q'||$}{
    ...
    \If{$P_{Q_i} \cup K_{Q_i} \neq \emptyset$}{
      $q \gets q' \in P_{Q_i} \cup K_{Q_i}$\;
      \If{$P_{Q_i} \neq \emptyset$ and $K_{Q_i} \neq \emptyset$ and $q \notin K_{Q_i}$}{
        $q \gets q_K \in K_{Q_i}$\;
      }
...
\begin{algo}{Algoritmo per l'identificazione degli stati kernel.}
  ...
  \If{$C \neq \emptyset$}{
    $q \gets q_C \in C$\;
    $K \gets depthFirstSearch(A, q)$\;
  }
  \Return{$K$}\;
\end{algo}

L'Algoritmo \ref{algo:hm-tarjan} corrisponde alla versione semplificata dell'algoritmo di Tarjan per
...
\begin{algo}{Algoritmo di Tarjan per l'identificazione degli stati centrali.}
  ...
  {Al termine dell'esecuzione dell'algoritmo, l'insieme $C$ contiene gli stati centrali}
  ...
  \ForEach{$\sigma \in \Sigma$}{
    \If{$containsKey(index, \delta(q, \sigma))$}{
...
siano già implementate alcune funzioni necessarie al loro utilizzo e che non sono state riportate in dettaglio. 
...
\begin{algo}{Algoritmo per il calcolo delle classi di quasi-equivalenza.}
  ...
  $H = \text{mappa vuota che associa vettori di successori a stati}$

  $R = \text{mappa vuota che associa ad uno stato la parte di cui è elemento}$\;
  ...
  \While{$I \neq \emptyset$}{
    ...
    \If{$q_A \in P$ and $containsKey(H, V(q_A))$}{
      $q_B = H[V(q_A)]$\;
