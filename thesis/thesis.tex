...
\begin{itemize}
  ...
  \item nel Capitolo 4 sono presentati i risultati sperimentali ottenuti dall'implementazione ed esecuzione
  degli algoritmi nella pratica.
\end{itemize}
...
\begin{algo}{Algoritmo di Tarjan per l'identificazione degli stati kernel.}
  ...
  \If{$low[q] = index[q]$}{
    ...
    \If{$||scc|| > 0$}{
      $V \gets depthFirstSearch(A, S.pop())$\;
      $K \gets K \cup V$\;
    }
  }
\end{algo}
...
La prima fase dello sviluppo è stata quella di individuare l'ambiente di sviluppo più adatto per l'implementazione
degli algoritmi di iperminimizzazione. Un'attenta analisi delle possibilità offerte dai diversi linguaggi di
programmazione e delle librerie messe a disposizione dall'ampia community open-source ha portato alla
scelta di \emph{Python 3.12.3} \cite{Python} come linguaggio principale per l'implementazione degli algoritmi di iperminimizzazione.
...
libreria \emph{Automata} \cite{Automata} per gestire i DFA all'interno del codice, questa
è stata scelta poiché implementa al suo interno l'algoritmo di minimizzazione di Hopcroft e permette la
visualizzazione degli automi sotto forma di grafi usando la libreria \emph{Graphviz}.
