...
\begin{algo}{Algoritmo per la costruzione dell'automa iperminimo.}
  ...
  \For{$i \gets 1$ \KwTo $||Q'||$}{
    ...
    \If{$P_{Q_i} \cup K_{Q_i} \neq \emptyset$}{
      $q \gets q' : q' \in P_{Q_i} \cup K_{Q_i}$\;
      \If{$P_{Q_i} \neq \emptyset$ and $K_{Q_i} \neq \emptyset$ and $q \notin K_{Q_i}$}{
        $q \gets q_K : q_K \in K_{Q_i}$\;
      }
...
\begin{algo}{Algoritmo per la costruzione dell'automa iperminimo.}
  ...
  \ForEach{$Q_i \in \partitioned{Q}{\sim}$}{
    ...
    \lIf{$K_{Q_i} \neq \emptyset$}{$q \gets q_K : q_K \in K_{Q_i}$}
    \lElse{$q \gets q_P : q_P \in P_{Q_i}$}
...
\begin{algo}{Algoritmo per l'identificazione degli stati kernel.}
  ...
  $C, K \gets \emptyset, \emptyset$\;
  $tarjan(A, q_I, index, low, i, S, C)$ \Comment*[r]{Algoritmo \ref{algo:hm-tarjan}}
  \ForEach{$q \in C$}{
    $V \gets depthFirstSearch(A, q)$\;
    $K \gets K \cup V$\;
  }
  \Return{$K$}\;
\end{algo}

L'Algoritmo \ref{algo:hm-tarjan} corrisponde alla versione semplificata dell'algoritmo di Tarjan \cite{DFI08} per
...
\begin{algo}{Algoritmo di Tarjan per l'identificazione degli stati centrali.}
  ...
  l'insieme $C$ nel quale vengono inseriti gli stati centrali}
  {Al termine dell'esecuzione dell'algoritmo, l'insieme $C$ conterrà gli stati centrali}
...
Un'attenta analisi delle possibilità offerte dai diversi linguaggi di programmazione e delle librerie
messe a disposizione dall'ampia community open-source ha portato alla scelta di \emph{Python 3.12.3}
come linguaggio principale per l'implementazione degli algoritmi di iperminimizzazione.

Nel dettaglio, si è scelto di utilizzare la versione di Python 3.12.3 e si è fatto uso della libreria 
\emph{automata-lib} per la rappresentazione e la manipolazione di automi a stati finiti
deterministici poiché essa offre out-of-the-box 
...
Seguendo questa strada, si è dovuta trovare una strategia per generare automi a stati finiti deterministici
...
per il generico stato $q_{i,j}$ corrispondente all'elemento in riga $i$ e colonna $j$ è definita
come segue: $\delta(q_{i,j}, a) = q_{i+1,j}$ se $i < \sqrt{n}$ e $\delta(q_{i,j}, b) = q_{1,j+1}$ se $j < \sqrt{n}$.
Si osservi che la funzione di transizione resta temporaneamente parziale in quanto non definita per gli stati
che si trovano nell'ultima riga della matrice ed il simbolo $a$ e per gli stati che si trovano nell'ultima colonna
della matrice ed il simbolo $b$, tali transizioni saranno poi sfruttate per portare dagli stati preambolo agli 
stati kernel. 
...
Un esempio di un automa a stati finiti deterministico $A$, con $||P(A)|| = 9$, $||K(A)|| = 3$ e kernel composto da una singola
componente fortemente connessa, generato seguendo la procedura descritta è mostrato in Figura \ref{fig:gen-dfa}.

Di seguito sono riportati i risultati ottenuti dai test effettuati. Questi ultimi sono stati svolti variando 
il rapporto tra gli stati preambolo e gli stati kernel, la percentuale di stati 
kernel finali, la composizione del kernel ed il numero di stati dell'automa.
...
La Figura \ref{fig:dfa-gen-fstates-pstates} mostra come varia l'efficacia dell'iperminimizzazione 
...
dimensioni ridotte contenente un alto numero di stati finali sono più proni all'essere iperminimizzati.
...
Si osserva come, ancora una volta nonostante all'aumentare del numero di stati diminuisca la percentuale di automi
iperminimizzabili, automi con un kernel composto da più di una componente fortemente connessa e con un alto
...
Sulla base delle osservazioni fatte in precedenza si effettuano i test per valutare le performance
degli algoritmi di iperminimizzazione implementati. In particolare, si è scelto di confrontare
le prestazioni degli algoritmi su un campione di automi diviso in gruppi di automi con un differente
numero di stati, da $2^1$ a $2^8$, ciascuno dei quali composto da $256$ automi con un kernel composto
da più di una componente fortemente connessa e con un rapporto tra stati preambolo e stati kernel ed
un numero di stati finali nel kernel pari a $0.8$.

I test sono stati effettuati su un computer MacBook Pro con processore M1, 16GB di RAM e sistema operativo
macOS Sonoma 14.5. I risultati ottenuti sono mostrati in Figura \ref{fig:dfa-pcomparison-line} e 
\ref{fig:dfa-pcomparison-violin}.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=1\linewidth]{images/performance_comparison_line.png}
  \caption{\label{fig:dfa-pcomparison-line}Grafico per la valutazione delle performance degli algoritmi di iperminimizzazione.}
\end{figure}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=1\linewidth]{images/performance_comparison_violin.png}
  \caption{\label{fig:dfa-pcomparison-violin}Grafico per la valutazione delle performance degli algoritmi di iperminimizzazione.}
\end{figure}

Si osservi come i risultati sperimentali ottenuti confermino le aspettative, l'algoritmo di Holzer e Maletti risulta essere
il più performante tra quelli implementati, seguito dall'algoritmo di Badr ed infine da quello di Badr, Geffert e Shipman.
